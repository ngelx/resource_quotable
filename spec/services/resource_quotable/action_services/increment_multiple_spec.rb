...
module ResourceQuotable
  module ActionServices
    RSpec.describe IncrementMultiple do
      ...
      describe 'call' do
        ...
        let(:tracker1) { build_stubbed(:quotum_tracker, quotum: quotum1, counter: 0) }
        let(:tracker2) { build_stubbed(:quotum_tracker, quotum: quotum2, counter: 0) }
        ...
        before do
          ...
          allow(tracker1).to receive(:increment_by!).with(20)
          allow(tracker2).to receive(:increment_by!).with(20)
          call
        end

        it { expect(tracker1).to have_received(:increment_by!).with(20) }
        it { expect(tracker2).to have_received(:increment_by!).with(20) }
      end
    end
  end
end
